...
'' \
'' \
'def test_authenticate_user_rejects_inactive_user(tmp_path, monkeypatch):' \
'    db_path = tmp_path / "test_credit.db"' \
'    monkeypatch.setattr(app, "DB_PATH", db_path)' \
'' \
'    conn = app.get_conn()' \
...
'' \
'    conn.execute(' \
'        "INSERT INTO usuarios (username, password_hash, rol, activo) VALUES (?, ?, ?, 0)",' \
'        ("bloqueado", app.hash_password("secreto123"), "admin"),' \
'    )' \
'' \
'    conn.commit()' \
'    conn.close()' \
'' \
'    assert app.authenticate_user("bloqueado", "secreto123") is None' \
'' \
'' \
'def test_authenticate_user_rejects_empty_credentials():' \
'    assert app.authenticate_user("", "abc") is None' \
'    assert app.authenticate_user("analista", "") is None' \
'    assert app.authenticate_user("", "") is None' \
